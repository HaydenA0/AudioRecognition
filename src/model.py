...
class EmotionDataset(Dataset):
    def __init__(self, csv_file, label_encoder=None):
        ...
        if label_encoder is None:
            ...
            self.labels = self.le.transform(self.df['label'])
...
def train():
    ...
    mapping = dict(zip(range(len(train_dataset.le.classes_)), train_dataset.le.classes_))
    ...
    for epoch in range(50):
        ...
        print(f"Epoch {epoch+1:02d}/50 | Loss: {total_loss/len(train_loader):.4f} | Test Acc: {current_acc:.2f}% | Best: {best_acc:.2f}%")
